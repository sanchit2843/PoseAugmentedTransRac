...
class MyData(Dataset):
    ...
    def __init__(self, root_path, video_path, label_path, num_frame):
        ...
        self.video_dir = os.listdir(
            os.path.join(self.root_path, self.video_path))
    ...
    def __getitem__(self, inx):
        ...
        file_path = os.path.join(
            self.root_path, self.video_path, video_file_name)
        ...
        if video_file_name in self.label_dict.keys():
            ...
            label = preprocess(video_frame_length, time_points,
...
def get_labels_dict(path):
    ...
    with open(path, encoding='utf-8') as f:
        ...
        for row in f_csv:
            cycle = [int(row[key])
                     for key in row.keys() if 'L' in key and row[key] != '']
