...
def plot_grad_flow(named_parameters):
    ...
    plt.legend(
        ...
        ["max-gradient", "mean-gradient", "median-gradient"],
    )
    plt.show()
...
def paint_smi_matrixs(matrixs, index=0):
    ...
    for i in range(c):
        ...
        plt.savefig(
            fname="graph/matrixs{0}/matrix{1}.png".format(index, str(i)), dpi=400
        )
        plt.close()
...
def density_map(maps, index, file_name):
    # paint density map
    plt.clf()
    ...
    plt.savefig(fname="density_map_test/{0}_{1}.png".format(file_name, index), dpi=500)
    plt.close()
    return cv2.imread("density_map_test/{0}_{1}.png".format(file_name, index))
