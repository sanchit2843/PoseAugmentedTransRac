...
def extract_keypoint_image(image, model, device="cuda"):
    ...
    image = letterbox(image, (image.shape[1]), stride=64, auto=True)[0]
    image_ = image.copy()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path", type=str, default="./yolov7-pose-estimation/yolov7-w6-pose.pt"
    )
    parser.add_argument(
        "--video_directory",
        type=str,
        default="/mnt/148C5DBB8C5D9856/UCF101/ucfrep",
    )
    parser.add_argument(
        "--output_directory",
        type=str,
    ...
    parser.add_argument(
        "--base_json", type=str, default="./yolov7-pose-estimation/base.json"
    )
    ...
    print(len(task_list))
    mmcv.track_parallel_progress(main, task_list, nproc=4)
