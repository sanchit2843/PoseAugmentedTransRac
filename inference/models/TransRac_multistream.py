...
class TransferModelMultiStream(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if pose:
            self.pose_model = TransferModelPose(num_frames = self.num_frames, scales = self.scales, OPEN = False,retrain = True)
            if training_flag:
                print("loading pose model from checkpoint")
                self.pose_model.load_state_dict(torch.load(kwargs["pose_checkpoint"])["state_dict"])
    ...
    def forward(self, x, heatmap=None):
        ...
        if self.num_classes:
            x_features = self.SpatialPooling(x_scale).squeeze(3).squeeze(3).transpose(1, 2)
            if self.pose:
                x_features_pose = self.SpatialPooling(mid_layer_outputs_pose["backbone"][-1]).squeeze(3).squeeze(3).transpose(1, 2)
        ...
        if self.pose:
            #x_scale += torch.stack(mid_layer_outputs_pose["conv3d_pooled"])
            x_scale = torch.cat([x_scale, mid_layer_outputs_pose["conv3d_pooled"][-1]], dim=2)
        ...
        if self.pose:    
            x += mid_layer_outputs_pose["input_projection"]
