    ...
    def forward(self, x):
        ...
        for scale in self.scales:
            if scale == 4:
                ...
                x = self.Replication_padding8(x)
                crops = [x[:, :, i:i + scale, :, :] for i in
                         range(0, self.num_frames - scale + scale // 2 * 2, max(scale // 2, 1))]
            elif scale == 32:
                x = self.Replication_padding16(x)
                crops = [x[:, :, i:i + scale, :, :] for i in
                         range(0, self.num_frames - scale + scale // 2 * 2, max(scale // 2, 1))]
            else:
                crops = [x[:, :, i:i + 1, :, :]
            ...
            # TODO: Make crop code run using single tensor instead of loop
            if not self.OPEN:
                with torch.no_grad():

                    # This code snippet can be used to extract features without loop
                    # f = len(crops)
                    # rearrange1 = rearrange(torch.stack(
                    #     crops), 'f b c t h w -> (f b) c t h w')

                    # slice = self.backbone(rearrange1)
                    # slice = rearrange(
                    #     slice, '(f b) c t h w -> f b c t h w', f=f)
                    # if slice.shape[3] % 2 == 0:
                    #     slice = slice[:, :, :, ::2, :, :] + \
                    #         slice[:, :, :, 1::2, :, :]
                    # slice = rearrange(slice, "f b c t h w -> b c (f t) h w")
                    ########################################################
            ...
            # x_scale = slice ## Uncomment if using the code snippet above, gives memory issues
            # ->[b,512,f,7,7]
        ...
        x_features = torch.sum(torch.stack(
            multi_scale_features), axis=0)  # [B,f,512*scale_num]
...
if __name__ == "__main__":
    ...
    my_model = TransferModelMovinet(config=config, checkpoint=checkpoint,
                                    num_frames=NUM_FRAME, scales=SCALES, OPEN=True, num_classes=None).cuda()
    import time
    st = time.time()
    my_model(torch.ones((8, 17, NUM_FRAME, 56, 56)).cuda())
    print(time.time()-st)
